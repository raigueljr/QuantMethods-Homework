...
The p-values in this Anova are the same as those in the summary above.  Except, it lumps disturb together to give the overall significance of disturbance.
```{r}
library(car)
...
The main OLS assumptions are listed below.  The plots for each model are below this discussion.
  ...
  *There is a linear response to the driver.
      *Since one of the exaplanatory variables is categorical (disturb) and the response variable (cover) is only integers (and therefore not continuous), there cannot be a linear response.
...
#####Question 2 
```{r}
#abies_ols is the same as abies_simpmod.  It is written using the glm function instead of lm,
#so the anova function can compare them.  Same is true of acer_ols.
...
The stepAIC function was used above in the exploratory analysis to compare to the chosen method conducted by hand.  The computer and I reached very similar conclusions for the simplified models of both species.