...
```{r}
plot(Variogram(sp_gls, form = ~ x + y))

sp_exp = update(sp_gls, 
                     corr=corExp(form=~x + y,
                                 nugget=T)) 

plot(Variogram(sp_exp, resType = 'normalized'))

```

```{r}
plot(Variogram(xylopia_gls, form= ~x + y))
```
...
plot(Variogram(xylopia_exp, resType='normalized'))
```
